    ...
    <ToastProvider>
      <div className="min-h-dvh bg-background text-foreground flex pb-[calc(4.5rem+env(safe-area-inset-bottom))] lg:pb-0">
        ...
        >
          ...
          <nav className="flex-1 overflow-y-auto py-4 px-3.5 space-y-1.5">
            {navItems.map((item) => {
              const isActive =
                pathname === item.href || pathname.startsWith(item.href + "/");
              const Icon = item.icon;
          ...
          <div className="shrink-0 border-t border-border/30 p-3.5">
            <LogoutForm
              className="px-3.5 py-3 rounded-xl text-sm font-bold hover:bg-muted/50"
